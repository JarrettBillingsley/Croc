...
private void register(MDThread* t, NativeFunc func, char[] name)
{
	newFunction(t, func, name);
	newGlobal(t, name);
}

private void constGlobal(MDThread* t, int x, char[] name)
{
	pushInt(t, x);
	newGlobal(t, name);
}

private void regField(MDThread* t, NativeFunc func, char[] name)
{
	newFunction(t, func, name);
	fielda(t, -2, name);
}

private void constField(MDThread* t, int x, char[] name)
{
	pushInt(t, x);
	fielda(t, -2, name);
}
...
static:
	void init(MDThread* t)
	{
		...
		makeModule(t, "sdl", function uword(MDThread* t)
		{
	...
	uword quit(MDThread* t)
	{
		SDL_Quit();
		return 0;
	}

	uword wasInit(MDThread* t)
	{
		pushInt(t, SDL_WasInit(cast(uword)checkIntParam(t, 1)));
		return 1;
	}

	uword getError(MDThread* t)
	{
		pushString(t, fromStringz(SDL_GetError()));
		return 1;
	}

	uword initSubSystem(MDThread* t)
	{
		auto flags = cast(uword)checkIntParam(t, 1);
	...
	uword quitSubSystem(MDThread* t)
	{
		auto flags = cast(uword)checkIntParam(t, 1);
	...
	uword glGetAttribute(MDThread* t)
	{
	...
	uword glSetAttribute(MDThread* t)
	{
		auto attr = cast(word)checkIntParam(t, 1);
	...
	uword glSwapBuffers(MDThread* t)
	{
		SDL_GL_SwapBuffers();
		return 0;
	}

	uword pushEvent(MDThread* t, ref SDL_Event ev)
	{
		switch(ev.type)
		{
	...
	void popEvent(MDThread* t, ref SDL_Event ev)
	{
	...
	uword evtPump(MDThread* t)
	{
		SDL_PumpEvents();
		return 0;
	}

	uword evtPoll(MDThread* t)
	{
		SDL_Event ev = void;
	...
	uword evtEnableUnicode(MDThread* t)
	{
		auto numParams = stackSize(t) - 1;
	...
	uword joyCount(MDThread* t)
	{
		pushInt(t, SDL_NumJoysticks());
		return 1;
	}
	
	uword joyOpen(MDThread* t)
	{
	...
	uword joyEnableEvents(MDThread* t)
	{
		auto numParams = stackSize(t) - 1;
	...
	uword imgQuit(MDThread* t)
	{
		IMG_Quit();
		return 0;
	}

	uword imgLoad(MDThread* t)
	{
		auto name = checkStringParam(t, 1);
...
static:
	...
	private SDL_Surface* getThis(MDThread* t)
	{
	...
	uword width(MDThread* t)
	{
		pushInt(t, getThis(t).w);
		return 1;
	}

	uword height(MDThread* t)
	{
		pushInt(t, getThis(t).h);
		return 1;
	}

	uword pitch(MDThread* t)
	{
	...
	uword pixels(MDThread* t)
	{
		pushInt(t, cast(mdint)getThis(t).pixels);
		return 1;
	}
	
	uword lock(MDThread* t)
	{
		auto s = getThis(t);
	...
	uword unlock(MDThread* t)
	{
		SDL_UnlockSurface(getThis(t));
		return 0;
	}

	uword free(MDThread* t)
	{
		checkInstParam(t, 0, "SdlSurface");
		auto psfc = cast(SDL_Surface**)getExtraBytes(t, 0).ptr;
