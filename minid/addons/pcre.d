...
static:
	...
	private void setSubject(MDThread* t, Members* memb, word str)
	{
		dup(t, str);
	...
	private word getNameTable(MDThread* t, pcre* re, pcre_extra* extra)
	{
		word numNames;
		word nameEntrySize;
		char* nameTable;
	...
	public uword numGroups(MDThread* t, uword numParams)
	{
		auto memb = getThis(t);
	...
	public uword groupNames(MDThread* t, uword numParams)
	{
		getThis(t);
	...
	public uword test(MDThread* t, uword numParams)
	{
		...
		(
			...
			memb.subject.length,
			memb.nextStart,
			PCRE_NO_UTF8_CHECK, // all MiniD strings are valid UTF-8
			memb.groupIdx.ptr,
	...
	public uword match(MDThread* t, uword numParams)
	{
		...
		if(numParams == 0)
		{
			...
			pushString(t, memb.subject[gi[0] .. gi[1]]);
		}
		else if(isInt(t, 1))
		{
			...
			pushString(t, memb.subject[gi[i * 2] .. gi[(i * 2) + 1]]);
		}
		else if(isString(t, 1))
		{
			...
			if(isNull(t, -1))
				throwException(t, "Invalid group name '{}'", getString(t, 1));

			auto i = cast(word)getInt(t, -1);
			pop(t, 2);
	...
	public uword pre(MDThread* t, uword numParams)
	{
		...
		pushString(t, memb.subject[0 .. memb.groupIdx[0]]);
		return 1;
	}
	...
	public uword post(MDThread* t, uword numParams)
	{
		...
		pushString(t, memb.subject[memb.groupIdx[1] .. $]);
		return 1;
	}
...
SharedLib load(char[][] paths)
{
	...
	foreach(path; paths)
	{
		...
		{
			try
				lib = SharedLib.load(FileSystem.getDirectory ~ path);
			catch (SharedLibException e) {}
		}
