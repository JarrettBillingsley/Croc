/******************************************************************************
License:
Copyright (c) 2007 ideage lsina@126.com
and Copyright (c) 2007 Jarrett Billingsley

This software is provided 'as-is', without any express or implied warranty.
In no event will the authors be held liable for any damages arising from the
use of this software.

Permission is granted to anyone to use this software for any purpose,
including commercial applications, and to alter it and redistribute it freely,
subject to the following restrictions:

    1. The origin of this software must not be misrepresented; you must not
	claim that you wrote the original software. If you use this software in a
	product, an acknowledgment in the product documentation would be
	appreciated but is not required.

    2. Altered source versions must be plainly marked as such, and must not
	be misrepresented as being the original software.

    3. This notice may not be removed or altered from any source distribution.
******************************************************************************/

module minid.regexplib;

import minid.types;
import minid.utils;

import std.regexp;

class RegexpLib
{
	this(MDNamespace namespace)
	{
		regexpClass = new MDRegexpClass();

		namespace.addList
		(
			"email"d,      r"\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"d,
			"url"d,        std.regexp.url,
			"alpha"d,      r"^[a-zA-Z_]+$"d,
			"space"d,      r"^\s+$"d,
			"digit"d,      r"^\d+$"d,
			"hexdigit"d,   r"^[0-9A-Fa-f]+$"d,
			"octdigit"d,   r"^[0-7]+$"d,
			"chinese"d,    "^[\u4e00-\u9fa5]+$"d,
			"cnPhone"d,    r"\d{3}-\d{8}|\d{4}-\d{7}"d,
			"cnMobile"d,   r"^((\(\d{2,3}\))|(\d{3}\-))?13\d{9}$"d,
			"cnZip"d,      r"^\d{6}$"d,
			"cnIDcard"d,   r"\d{15}|\d{18}"d,
			"symbol"d,     r"^[\(\)\[\]\.,;=<>\+\-\*/&\^]+$"d,
			"usPhone"d,    r"^((1-)?\d{3}-)?\d{3}-\d{4}$"d,
			"usZip"d,      r"^\d{5}$"d,

			"compile"d,    new MDClosure(namespace, &compile,    "regexp.compile"),
			"test"d,       new MDClosure(namespace, &test,       "regexp.test"),
			"replace"d,    new MDClosure(namespace, &replace,    "regexp.replace"),
			"split"d,      new MDClosure(namespace, &split,      "regexp.split"),
			"match"d,      new MDClosure(namespace, &match,      "regexp.match")
		);
	}

	int test(MDState s)
	{
		char[] pattern = s.getStringParam(0).asUTF8();
		char[] src = s.getStringParam(1).asUTF8();
		char[] attributes = "";
		
		if(s.numParams() > 2)
			attributes = s.getStringParam(2).asUTF8();

		s.push(s.safeCode(cast(bool)RegExp(pattern, attributes).test(src)));
		return 1;
	}

	int replace(MDState s)
	{
		char[] pattern = s.getStringParam(0).asUTF8();
		char[] src = s.getStringParam(1).asUTF8();
		char[] rep = s.getStringParam(2).asUTF8();
		char[] attributes = "";
		
		if(s.numParams() > 3)
			attributes = s.getStringParam(3).asUTF8();

		s.push(s.safeCode(RegExp(pattern, attributes).replace(src,rep)));
		return 1;
	}

	int split(MDState s)
	{
		char[] pattern = s.getStringParam(0).asUTF8();
		char[] src = s.getStringParam(1).asUTF8();
		char[] attributes = "";
		
		if(s.numParams() > 2)
			attributes = s.getStringParam(2).asUTF8();

		s.push(MDArray.create(s.safeCode(RegExp(pattern, attributes).split(src))));
		return 1;
	}

	int match(MDState s)
	{
		char[] pattern = s.getStringParam(0).asUTF8();
		char[] src = s.getStringParam(1).asUTF8();
		char[] attributes = "";
		
		if(s.numParams() > 2)
			attributes = s.getStringParam(2).asUTF8();

		s.push(MDArray.create(s.safeCode(RegExp(pattern, attributes).match(src))));
		return 1;
	}

	int compile(MDState s)
	{
		char[] pattern = s.getStringParam(0).asUTF8();
		char[] attributes = "";

		if(s.numParams() > 1)
			attributes = s.getStringParam(1).asUTF8();

		s.push(s.safeCode(regexpClass.newInstance(pattern, attributes)));
		return 1;
	}

	public MDRegexpClass regexpClass;

	static class MDRegexpClass : MDClass
	{
		public this()
		{
			super("Regexp", null);

			iteratorClosure = new MDClosure(mMethods, &iterator, "Regexp.iterator");

			mMethods.addList
			(
				"test"d,    new MDClosure(mMethods, &test,    "Regexp.test"),
				"search"d,  new MDClosure(mMethods, &search,  "Regexp.search"),
				"match"d,   new MDClosure(mMethods, &match,   "Regexp.match"),
				"pre"d,     new MDClosure(mMethods, &pre,     "Regexp.pre"),
				"post"d,    new MDClosure(mMethods, &post,    "Regexp.post"),
				"find"d,    new MDClosure(mMethods, &find,    "Regexp.find"),
				"split"d,   new MDClosure(mMethods, &split,   "Regexp.split"),
				"replace"d, new MDClosure(mMethods, &replace, "Regexp.replace") ,
				"opApply"d, new MDClosure(mMethods, &apply,   "Regexp.opApply")
			);
		}

		public override MDRegexp newInstance()
		{
			return new MDRegexp(this);
		}

		protected MDRegexp newInstance(char[] pattern, char[] attributes)
		{
			MDRegexp n = newInstance();
			n.constructor(pattern,attributes);
			return n;
		}

		protected MDRegexp newInstance(RegExp rxp)
		{
			MDRegexp n = newInstance();
			n.constructor(rxp);
			return n;
		}

		public int test(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(r.test(s.getStringParam(0).asUTF8()));
			return 1;
		}

		public int match(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();

			if(s.isParam!("int")(0))
				s.push(r.match(s.getIntParam(0)));
			else
				s.push(MDArray.create(r.match(s.getStringParam(0).asUTF8())));

			return 1;
		}

		public int search(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			char[] str = s.getStringParam(0).asUTF8();

			r.search(str);
			s.push(r);
			return 1;
		}

		public int pre(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(r.pre());
			return 1;
		}

		public int post(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(r.post());
			return 1;
		}

		public int replace(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(r.replace(s.getStringParam(0).asUTF8(), s.getStringParam(1).asUTF8()));
			return 1;
		}

		public int split(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();

			s.push(MDArray.create(r.split(s.getStringParam(0).asUTF8())));
			return 1;
		}

		public int find(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(r.find(s.getStringParam(0).asUTF8()));
			return 1;
		}

		MDClosure iteratorClosure;

		int iterator(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			int index = s.getIntParam(0) + 1;

			if(!r.test())
				return 0;

			s.push(index);
			s.push(r);
			return 2;
		}

		public int apply(MDState s)
		{
			MDRegexp r = cast(MDRegexp)s.getContext().asInstance();
			s.push(iteratorClosure);
			s.push(r);
			s.push(-1);
			return 3;
		}
	}

	static class MDRegexp : MDInstance
	{
		protected RegExp mRegexp;

		public this(MDClass owner)
		{
			super(owner);
		}

		public void constructor(char[] pattern, char[] attributes)
		{
			mRegexp = new std.regexp.RegExp(pattern, attributes);
		}

		public void constructor(RegExp rxp)
		{
			mRegexp = rxp;
		}

		char[] match(uint n)
		{
			return mRegexp.match(n);
		}

		char[] pre()
		{
			return mRegexp.pre();
		}

		char[] post()
		{
			return mRegexp.post();
		}

		public bool test(char[] str)
		{
			return cast(bool)mRegexp.test(str);
		}
		
		public bool test()
		{
			return cast(bool)mRegexp.test();
		}

		int find(char[] str)
		{
			return mRegexp.find(str);
		}

		char[][] split(char[] str)
		{
			return mRegexp.split(str);
		}

		char[][] match(char[] str)
		{
			return mRegexp.match(str);
		}

		char[] replace(char[] str, char[] format)
		{
			return mRegexp.replace(str,format);
		}
		
		void search(char[] str)
		{
			mRegexp.search(str);
		}
	}
}

public void init()
{
	MDNamespace namespace = new MDNamespace("regexp"d, MDGlobalState().globals);
	new RegexpLib(namespace);
	MDGlobalState().setGlobal("regexp"d, namespace);
}
