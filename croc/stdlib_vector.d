...
static:
	...
	word get_i8(CrocThread* t, Members* memb, uword idx)             { return pushInt(t, (cast(byte*)memb.data)[idx]); }
	void set_i8(CrocThread* t, Members* memb, uword idx, word item)  { (cast(byte*)memb.data)[idx] = cast(byte)getInt(t, item); }
	word get_i16(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(short*)memb.data)[idx]); }
	void set_i16(CrocThread* t, Members* memb, uword idx, word item) { (cast(short*)memb.data)[idx] = cast(short)getInt(t, item); }
	word get_i32(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(int*)memb.data)[idx]); }
	void set_i32(CrocThread* t, Members* memb, uword idx, word item) { (cast(int*)memb.data)[idx] = cast(int)getInt(t, item); }
	word get_i64(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(long*)memb.data)[idx]); }
	void set_i64(CrocThread* t, Members* memb, uword idx, word item) { (cast(long*)memb.data)[idx] = cast(long)getInt(t, item); }

	word get_u8(CrocThread* t, Members* memb, uword idx)             { return pushInt(t, (cast(ubyte*)memb.data)[idx]); }
	void set_u8(CrocThread* t, Members* memb, uword idx, word item)  { (cast(ubyte*)memb.data)[idx] = cast(ubyte)getInt(t, item); }
	word get_u16(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(ushort*)memb.data)[idx]); }
	void set_u16(CrocThread* t, Members* memb, uword idx, word item) { (cast(ushort*)memb.data)[idx] = cast(ushort)getInt(t, item); }
	word get_u32(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(uint*)memb.data)[idx]); }
	void set_u32(CrocThread* t, Members* memb, uword idx, word item) { (cast(uint*)memb.data)[idx] = cast(uint)getInt(t, item); }
	word get_u64(CrocThread* t, Members* memb, uword idx)            { return pushInt(t, (cast(long*)memb.data)[idx]); }
	void set_u64(CrocThread* t, Members* memb, uword idx, word item) { (cast(long*)memb.data)[idx] = cast(long)getInt(t, item); }

	word get_f32(CrocThread* t, Members* memb, uword idx)            { return pushFloat(t, (cast(float*)memb.data)[idx]); }
	void set_f32(CrocThread* t, Members* memb, uword idx, word item) { (cast(float*)memb.data)[idx] = cast(float)getNum(t, item); }
	word get_f64(CrocThread* t, Members* memb, uword idx)            { return pushFloat(t, (cast(double*)memb.data)[idx]); }
	void set_f64(CrocThread* t, Members* memb, uword idx, word item) { (cast(double*)memb.data)[idx] = cast(double)getNum(t, item); }

	struct TypeStruct
	{
		TypeCode code;
		ubyte itemSize;
		word function(CrocThread* t, Members* memb, uword idx) getItem;
		void function(CrocThread* t, Members* memb, uword idx, word item) setItem;
	}
	...
	*/
	public word fromDArray(_T)(CrocThread* t, _T[] arr)
	{
		alias realType!(_T) T;
	...
	Returns:
		The stack index of the newly-pushed Vector instance.
	*/
	public word viewDArray(_T)(CrocThread* t, _T[] arr)
	{
		alias realType!(_T) T;
	...
	*/
	public void reviewDArray(_T)(CrocThread* t, word slot, _T[] arr)
	{
		alias realType!(_T) T;
	...
	void fillImpl(CrocThread* t, Members* memb, word idx, uword lo, uword hi)
	{
		pushGlobal(t, "Vector");
	...
	uword max(CrocThread* t)
	{
		...
		switch(memb.type.code)
		{
			...
			case TypeCode.u64: pushInt(t, cast(crocint)minMaxImpl!(">")(cast(ulong*)memb.data, memb.length)); break;
	...
	uword rawRead(T)(CrocThread* t)
	{
		...
		static if(isIntegerType!(T))
			pushInt(t, cast(crocint)*(cast(T*)(data.ptr + idx)));
		else
			pushFloat(t, cast(crocfloat)*(cast(T*)(data.ptr + idx)));
