...
version(CrocBuiltinDocs)
{
	...
	[
		{kind: "function", name: "collect", docs:
		"Performs a normal garbage collection cycle. Usually you won't have to call this because the GC will be run
		periodically on its own, but sometimes it's useful to force a cycle. Returns the number of bytes reclaimed by
		the GC."},

		{kind: "function", name: "collectFull", docs:
		...
		collected, so it can be useful. Returns the number of bytes reclaimed by the GC."},

		{kind: "function", name: "allocated", docs:
		"Returns the total bytes currently allocated by this VM instance."},

		{kind: "function", name: "limit", docs:
		...
		If only called with a `type` parameter and no `size` parameter, gets the given limit. If a `size` parameter is passed, sets
		the given limit to that size and returns the previously-set limit.

		The `type` parameter is a string that must be one of the following values:

 * \"nurseryLimit\" - The size, in bytes, of the nursery generation. Defaults to 512KB. Most objects are initially
 ...
 * \"metadataLimit\" - The size, in bytes, of the GC metadata. Defaults to 128KB. The metadata includes two buffers: one
 ...
 * \"nurserySizeCutoff\" - The maximum size, in bytes, of an object that can be allocated in the nursery. Defaults to 256.
 ...
 * \"cycleCollectInterval\" - Since the Croc reference implementation uses a form of reference counting to do garbage
   ...
   buffered (see the next limit type for that), or every ''n'' collections, whichever comes first. This limit is that ''n''.
 ...
 * \"cycleMetadataLimit\" - As explained above, the GC will buffer potential cyclic garbage during normal GC cycles, and then
   ...
   Thus the cycle collector must be run to reclaim ALL dead objects.",
		...
		{kind: "function", name: "postCallback", docs:
		...
		{kind: "function", name: "removePostCallback", docs:
		"Removes a post-GC callback function that was previously added with `postCallback`. If the given function is not in the list
