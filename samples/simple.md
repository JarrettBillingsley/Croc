...
object Matrix
{
	...
	function clone(vararg)
	{
		if(#vararg == 0)
			throw "Expected at least one argument"

		local data = array.new(#vararg)
		...
		for(i: 1 .. #vararg)
		{
			if(!isArray(vararg[i]) || #vararg[i] != len)
	...
	{
		local rows = s.split(";").apply(\v -> v.strip())
		local data = array.new(#rows)

		foreach(i, row; rows)
		{
			local cols = row.split(",").apply(\v -> v.strip())
			local colData = array.new(#cols)

			foreach(j, col; cols)
				try
					colData[j] = toFloat(col)
				catch(e)
					throw "All data must be nubmers"

			data[i] = colData
		}

		return :clone(data.expand())
	}
	...
	function rref()
	{
		...
		for(r: 0 .. numRows)
		{
			...
			:swapRows(i, r)
			:scaleRow(r, 1.0 / data[r][lead])

			for(j: 0 .. numRows)
				if(j != r)
					:addRows(j, r, -data[j][lead])

			lead++
		}
	}

	function swapRows(i: int, j: int)
	{
		if(i == j)
			return
	...
	function scaleRow(i: int, a: int|float)
	{
		local rowi = :mData[i]
	...
	function addRows(i: int, j: int, a: int|float)
	{
		local rowi = :mData[i]
		local rowj = :mData[j]
