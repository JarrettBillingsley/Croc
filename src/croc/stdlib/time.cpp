...
word_t _microTime(CrocThread* t)
{
...
word_t _clockTime(CrocThread* t)
{
...
word_t _timeToTableUTC(CrocThread* t)
{
...
word_t _timeToTableLocal(CrocThread* t)
{
	auto time = croc_ex_optParam(t, 1, CrocType_Int) ? cast(oscompat::Time)croc_getInt(t, 1) : oscompat::sysTime();
	auto ret = croc_ex_optParam(t, 2, CrocType_Table) ? 2 : croc_table_new(t, 0);
...
word_t _timeFromTableUTC(CrocThread* t)
{
...
const StdlibRegisterInfo _timeFromTableLocal_info =
{
	...
	"timeFromTableLocal", 1
};

word_t _timeFromTableLocal(CrocThread* t)
{
	croc_ex_checkParam(t, 1, CrocType_Table);
	oscompat::DateTime dt;
	_TableToDateTime(t, 1, dt);
...
const StdlibRegister _globalFuncs[] =
{
	...
	_DListItem(_timex),
	_DListItem(_clockTime),
	_DListItem(_timeToTableUTC),
	_DListItem(_timeToTableLocal),
	_DListItem(_timeFromTableUTC),
	_DListItem(_timeFromTableLocal),
	_DListEnd
};
